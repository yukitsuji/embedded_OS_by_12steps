/*
 *シリアルデバイス用のデバイスドライバ
 H8/3069Fマイコン・ボードはシリアル経由で操作するので、シリアル入出力機能は必須
 */
...
//各種レジスタの定義 uint8 8ビット長の符号なし
struct h8_3069f_sci{
  volatile uint8 smr;//シリアル通信のモード設定
  volatile uint8 brr;//ポートレートの設定
  volatile uint8 scr;//送受信の有効,無効
  volatile uint8 tdr;//送信したい一文字を書き込む
  volatile uint8 ssr;//送信完了,受信完了を表す
  volatile uint8 rdr;//受信した一文字を読みだす
  volatile uint8 scmr;
};
//SMRの各ビットの定義
...
int serial_init(int index){
  ...
  sci->scr = 0;
  sci->smr = 0;
  sci->brr = 64;//20MHzのクロックから9600bpsを生成(一秒間に9600bit送信)
